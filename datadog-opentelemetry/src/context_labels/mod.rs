...
#[cfg(feature = "context-observer")]
pub fn init_custom_labels_context() {
    let writer = CustomLabelsWriter::new();
    let observer = Arc::new(ContextLabelObserver::new(writer));
    GlobalContextObserver::set(observer);
}

// Fire it up with a custom writer
#[cfg(feature = "context-observer")]
pub fn init_custom_labels_context_with(writer: CustomLabelsWriter) {
    let observer = Arc::new(ContextLabelObserver::new(writer));
    GlobalContextObserver::set(observer);
}

pub fn init_context_labels<W: ContextLabelWriter>(writer: W) {
